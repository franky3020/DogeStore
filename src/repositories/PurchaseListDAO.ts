...
export default class PurchaseListDAO {
  ...
  insertProductToList(
    ...
    let purchase_time_Date = purchase_time
      .toISOString()
      .slice(0, 19)
      .replace("T", " ");

    let sql =
      "INSERT INTO `PurchaseList`(`user_id`, `product_id`, `purchase_time`)VALUES(?,?,?)";
    return this.connection
      .promise()
      .query(sql, [user_id, product_id, purchase_time_Date]);
  }
  ...
  findUserPurchase(user_id: number): Promise<Product[]> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        let sql =
          "SELECT `Products`.* FROM `PurchaseList` INNER JOIN `Products` \
      ...
      for (const product of jResult as Product[]) {
        // TODO: 有重複問題 insert 時就要檢查了, 且改成傳回 id 就好 保持簡單
        returnProducts.push(
          new Product(
            product.id,
            product.name,
  ...
  deletePurchaseInList(user_id: number, product_id: number): Promise<any> {
    let sql =
      "DELETE FROM `PurchaseList` WHERE `user_id` = ? AND `product_id` = ?";

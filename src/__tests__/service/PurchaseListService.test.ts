...
test("Check user has product", async () => {
  ...
  mockPurchaseListDAO.findUserPurchase.mockReturnValue([
    ...
    new Product(
      product_init_2.id,
      product_init_2.name,
      product_init_2.create_user_id,
      product_init_2.price,
      product_init_2.describe,
    ),
  ]);

  const purchaseListService = new PurchaseListService(
    mockPurchaseListDAO as any,
  );

  let isUserHas = await purchaseListService.checkUserhasProduct(
    1,
    product_init_1.id,
  );
...
test("Get user purchase list", async () => {
  ...
  expect(json_result).toEqual([product_init_1.id, product_init_2.id]);
});

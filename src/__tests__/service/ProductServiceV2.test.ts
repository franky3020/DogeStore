  ...
  let mockProductDAO = {
    findById: jest.fn((id: number) => {
      return new Product(
        id,
        product_init_1.name,
        product_init_1.create_user_id,
        product_init_1.price,
        product_init_1.describe,
      );
    }),
  };

  const productService = new ProductService(
    mockProductDAO as unknown as ProductDAO,
  );
...
test("Add a product image and delete", async () => {
  ...
  let isImgExist = fs.existsSync(
    path.join(
      ProductService.SAVE_PRODUCT_IMAGES_PATH,
      createProductId.toString(),
      ProductService.PRODUCT_IMG_FILE_NAME,
    ),
  );
...
test("delete product", async () => {
  ...
  let mockDeleteZipFileFn = productService.deleteProductZipFile as jest.Mock<
    any,
    any
  >;
...
test("Add product zip file", async () => {
  ...
  await productService.addProductZipFile(
    createProductId,
    Buffer.from(zipFilestr),
  );

  let isZipFileExist = fs.existsSync(
    path.join(
      ProductService.SAVE_PRODUCT_ZIP_FILE_PATH,
      createProductId.toString(),
      ProductService.PRODUCT_ZIP_FILE_NAME,
    ),
  );

...
class PurchaseRouter {
  ...
  intializeRoutes() {
    ...
    this.router
      .route("/")
      .get(authentication, this.getUserPurchaseList.bind(this));
    this.router
      .route("/productZipFile/:id")
      .get(
        authentication,
        this.isUserhasProductMiddleware.bind(this),
        this.getProductZipFile.bind(this),
      );
    ...
    this.router
      ...
      .post(
        authentication,
        validate(this.purchaseValidation),
        this.addPurchasetList.bind(this),
      );
    this.router
      .route("/productZipFile/:id")
      .post(
        authentication,
        isAdminMiddleware,
        this.uploadFile.single("uploaded_file"),
        this.uploadProductZipFile.bind(this),
      );
  }
  ...
  async addPurchasetList(req: any, res: Response, next: NextFunction) {
    try {
      ...
      await this.purchaseListService.addProductToPurchaseList(
        user_id,
        product_id,
      );
  ...
  async getUserPurchaseList(req: any, res: Response, next: NextFunction) {
    try {
      let user_id = req.authUserID;
      let products_id_json =
        await this.purchaseListService.getUserPurchaseList(user_id);
  ...
  async isUserhasProductMiddleware(
    req: any,
    res: Response,
    next: NextFunction,
  ) {
    try {
      ...
      let isUserhasProduct = await this.purchaseListService.checkUserhasProduct(
        user_id,
        product_id,
      );
